...
def resave_patched_table(table_name, lookup_cache, config, output_dir, loaded_lookup_names=None):
    """Helper to re-save a table's CSV after its data has been updated in the lookup_cache."""
    if loaded_lookup_names is None:
        loaded_lookup_names = set()

    # Skip re-saving loaded lookup tables
    if table_name in loaded_lookup_names:
        print(f"✅ Skipping re-save of loaded lookup table '{table_name}'.")
        return
...
def _save_lookup_tables(lookup_cache, tables, output_dir):
    for table in tables:
        table_name = table.get('name')
        rows = lookup_cache.get(table_name)
        ...
        if not columns:
            continue
        csv_path = os.path.join(output_dir, f"{table_name}.csv")
        save_table_to_csv(rows, columns, csv_path)
        print(f"💾 Saved lookup CSV for table '{table_name}' ➜ {csv_path}")


def _load_lookup_csv(csv_path):
    df = pd.read_csv(csv_path)
    df = df.where(pd.notna(df), None)
    return df.to_dict(orient='records')


def _load_id_state(path):
...
def _save_id_state(path, id_state):
    dir_name = os.path.dirname(path)
    if dir_name:
        os.makedirs(dir_name, exist_ok=True)
    with open(path, 'w') as f:
        json.dump(id_state, f, indent=2)
...
def main():
    ...
    parser.add_argument('--start-date', type=str, default=None, help='Start date for data generation (YYYY-MM-DD)')
    parser.add_argument('--end-date', type=str, default=None, help='End date for data generation (YYYY-MM-DD)')
    parser.add_argument('--load-lookups-from', type=str, default=None, help='Directory with customers.csv and product_catalog.csv')
    parser.add_argument('--generate-lookups-only', action='store_true', help='Generate lookup tables only and exit')
    parser.add_argument('--id-state-file', type=str, default=None, help='Path to JSON file to persist sequential IDs')
    ...
    if faker_seed is None:
        faker_seed = config.get_parameter('faker_seed', None)
    faker_instance = Faker()
    if faker_seed is not None:
        Faker.seed(faker_seed)
        faker_instance.seed_instance(faker_seed)
        random.seed(faker_seed)
        np.random.seed(faker_seed)

    # --- NEW: Define Global Date Range for Data Generation ---
    # Allow override via CLI arguments, otherwise use current date and config
    if args.start_date and args.end_date:
        try:
            global_start_date = datetime.strptime(args.start_date, '%Y-%m-%d').date()
            global_end_date = datetime.strptime(args.end_date, '%Y-%m-%d').date()
            print(f"📅 Using CLI-provided date range: {global_start_date} to {global_end_date}")
        except ValueError as e:
            print(f"❌ Invalid date format. Expected YYYY-MM-DD. Error: {e}")
            sys.exit(1)
    else:
        ...
        print(f"📅 Using default date range: {global_start_date} to {global_end_date} ({order_days_back} days)")
    ...
    loaded_lookup_names = set()
    if args.load_lookups_from:
        customers_path = os.path.join(args.load_lookups_from, "customers.csv")
        product_catalog_path = os.path.join(args.load_lookups_from, "product_catalog.csv")

        if os.path.exists(customers_path):
            lookup_cache["customers"] = _load_lookup_csv(customers_path)
            loaded_lookup_names.add("customers")
            print(f"✅ Loaded customers from {customers_path}")
        else:
            print(f"⚠️ Customers lookup not found at {customers_path}, will generate.")

        if os.path.exists(product_catalog_path):
            lookup_cache["product_catalog"] = _load_lookup_csv(product_catalog_path)
            loaded_lookup_names.add("product_catalog")
            print(f"✅ Loaded product catalog from {product_catalog_path}")
        else:
            print(f"⚠️ Product catalog lookup not found at {product_catalog_path}, will generate.")
    ...
    if lookup_config:
        ...
        for lookup_name, lookup_params in lookup_config.items():
            if lookup_name in loaded_lookup_names:
                print(f"✅ Using loaded lookup for '{lookup_name}', skipping generation.")
                continue
    ...
    if args.generate_lookups_only:
        _save_lookup_tables(lookup_cache, config.tables, output_dir)
        print("✅ Lookup-only generation complete.")
        return

    id_state = None
    if args.id_state_file:
        id_state = _load_id_state(args.id_state_file)
        from generators.generator_carts import initialize_cart_id_counter, get_final_cart_id_counter
        from generators.generator_orders import initialize_order_id_counter, get_final_order_id_counter
        from generators.generator_returns import initialize_return_id_counter, get_final_return_id_counter
        initialize_cart_id_counter(id_state.get("last_cart_id", 0))
        initialize_order_id_counter(id_state.get("last_order_id", 0))
        initialize_return_id_counter(id_state.get("last_return_id", 0))
    ...
    for table in tables:
        ...
        if rows is not None and not (hasattr(rows, 'empty') and rows.empty):
            # Skip saving lookup tables that were loaded from external source
            # They should remain in the static lookups directory, not be copied to chunk output
            if table_name in loaded_lookup_names:
                print(f"✅ Using loaded lookup for '{table_name}', skipping CSV save to chunk output.")
            else:
                csv_path = os.path.join(output_dir, f"{table_name}.csv")
        ...
        if table_name == "cart_items":
            ...
            resave_patched_table("shopping_carts", lookup_cache, config, output_dir, loaded_lookup_names)
            ...
            if carts:
                ...
                resave_patched_table("cart_items", lookup_cache, config, output_dir, loaded_lookup_names)

        if table_name == "return_items":
            resave_patched_table("returns", lookup_cache, config, output_dir, loaded_lookup_names)

        if table_name == "order_items":
            resave_patched_table("orders", lookup_cache, config, output_dir, loaded_lookup_names)
    ...
    # Note: If customers were loaded from static lookups, they won't be re-saved
    resave_patched_table("customers", lookup_cache, config, output_dir, loaded_lookup_names)
    resave_patched_table("orders", lookup_cache, config, output_dir, loaded_lookup_names)

    if args.id_state_file and id_state is not None:
        from generators.generator_carts import get_final_cart_id_counter
        from generators.generator_orders import get_final_order_id_counter
        from generators.generator_returns import get_final_return_id_counter
        id_state['last_cart_id'] = get_final_cart_id_counter()
        id_state['last_order_id'] = get_final_order_id_counter()
        id_state['last_return_id'] = get_final_return_id_counter()
        id_state['last_updated'] = datetime.utcnow().isoformat(timespec='seconds') + "Z"
        _save_id_state(args.id_state_file, id_state)
        print(f"🧾 Updated ID state file ➜ {args.id_state_file}")
