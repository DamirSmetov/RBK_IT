...
_return_id_counter = None

def initialize_return_id_counter(start_value: int) -> None:
    global _return_id_counter
    _return_id_counter = start_value


def generate_return_id(faker_instance: Faker) -> str:
    """Generate a unique return ID."""
    global _return_id_counter
    if _return_id_counter is not None:
        _return_id_counter += 1
        return f"RTN-{_return_id_counter:08d}"
    if faker_instance is None:
        raise ValueError("faker_instance is required when return ID counter is not initialized.")
    return faker_instance.unique.bothify(text="RTN-########")


def get_final_return_id_counter() -> int:
    return _return_id_counter if _return_id_counter is not None else 0
