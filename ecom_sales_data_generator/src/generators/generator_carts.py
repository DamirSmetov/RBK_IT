...
_cart_id_counter = None

def initialize_cart_id_counter(start_value: int) -> None:
    global _cart_id_counter
    _cart_id_counter = start_value


def generate_cart_id(faker_instance: Faker) -> str:
    """Generate a unique cart ID."""
    global _cart_id_counter
    if _cart_id_counter is not None:
        _cart_id_counter += 1
        return f"CART-{_cart_id_counter:08d}"
    if faker_instance is None:
        raise ValueError("faker_instance is required when cart ID counter is not initialized.")
    return faker_instance.unique.bothify(text="CART-########")


def get_final_cart_id_counter() -> int:
    return _cart_id_counter if _cart_id_counter is not None else 0
