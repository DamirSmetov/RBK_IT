...
_order_id_counter = None

def initialize_order_id_counter(start_value: int) -> None:
    global _order_id_counter
    _order_id_counter = start_value


def generate_order_id(faker_instance: Faker) -> str:
    """Generate a unique order ID."""
    global _order_id_counter
    if _order_id_counter is not None:
        _order_id_counter += 1
        return f"ORD-{_order_id_counter:08d}"
    if faker_instance is None:
        raise ValueError("faker_instance is required when order ID counter is not initialized.")
    return faker_instance.unique.bothify(text="ORD-########")


def get_final_order_id_counter() -> int:
    return _order_id_counter if _order_id_counter is not None else 0
